# 03 · Direct WebRTC into a Daily or LiveKit room

Speak a WAV file through an Ojin avatar that joins **your** Daily or LiveKit room. The inference server joins as a participant named `ojin-avatar` and publishes the avatar's audio and video there, so anyone in the room sees and hears it — no media flows through this script.
...
export OJIN_WEBRTC_PROVIDER="daily"  # or "livekit"
...
Use your LiveKit server URL (`wss://<project>.livekit.cloud`) as `OJIN_WEBRTC_ROOM_URL`, and mint an access token whose identity is **exactly `ojin-avatar`**:
...
- There is no fallback: if the avatar can't join, the script exits with a `WEBRTC_JOIN_FAILED` or `WEBRTC_UNSUPPORTED` error.

...
def test_flood_strands_the_cancel_behind_buffered_audio() -> None:
    ...
    r = asyncio.run(run_experiment(n_chunks=_N_CHUNKS, consume_s=_CONSUME_S, pace_audio=False))
...
def test_pacing_audio_to_realtime_delivers_the_cancel_promptly() -> None:
    ...
    r = asyncio.run(run_experiment(n_chunks=_N_CHUNKS, consume_s=_CONSUME_S, pace_audio=True))
    assert r["cancel_latency_s"] is not None
...
def test_pacing_is_dramatically_faster_than_flooding() -> None:
    ...
    flood = asyncio.run(run_experiment(n_chunks=_N_CHUNKS, consume_s=_CONSUME_S, pace_audio=False))
    paced = asyncio.run(run_experiment(n_chunks=_N_CHUNKS, consume_s=_CONSUME_S, pace_audio=True))
